...
class PluginController extends BaseAdminController
{
    ...
    /**
     * POST /admin/plugin/amocrm/pipelines — Fetch pipelines from AmoCRM API
     * Cached for 5 minutes to avoid hammering AmoCRM on every UI click.
     */
    public function actionAmocrmPipelines(): array
    {
        Yii::$app->response->format = Response::FORMAT_JSON;
        $this->requirePermission('manageSettings');
        ...
        try {
            $cache    = Yii::$app->cache;
            $cacheKey = 'amocrm.pipelines.v1';
            $cached   = $cache->get($cacheKey);
            if (is_array($cached)) {
                return ['success' => true, 'pipelines' => $cached, 'cached' => true];
            }

            $pipelines = Yii::$app->amocrm->getPipelinesWithStatuses();
            if ($pipelines === null) {
                return ['success' => false, 'message' => 'AmoCRM не сконфигурирован (нет domain/access_token)'];
            }

            $cache->set($cacheKey, $pipelines, 300); // 5 min
            return ['success' => true, 'pipelines' => $pipelines, 'cached' => false];
        } catch (\Throwable $e) {
            Yii::error('[AmoCRM] pipelines fetch failed: ' . $e->getMessage(), 'amocrm');
            return ['success' => false, 'message' => $e->getMessage()];
        }
    }

    /**
     * POST /admin/plugin/amocrm/status-map-save — Save status mapping (validated server-side)
     */
    public function actionAmocrmStatusMapSave(): array
    {
        Yii::$app->response->format = Response::FORMAT_JSON;
        $this->requirePermission('manageSettings');
        ...
        if (!is_array($mappings) || empty($mappings)) {
            // Защита от случайной зачистки таблицы пустым POST-ом.
            return ['success' => false, 'message' => 'Пустой маппинг отвергнут (для очистки используйте отдельную операцию)'];
        }

        try {
            $count = \app\backend\modules\admin\services\AmocrmStatusMapper::saveMappings($mappings);
            // Audit
            $this->logAction(
                'amocrm_status_mapping_updated',
                'amocrm',
                null,
                'AmoCRM status mapping',
                'Saved ' . $count . ' mapping(s)',
                null,
                ['count' => $count]
            );
            ...
            Yii::error('[AmoCRM] saveMappings failed: ' . $e->getMessage(), 'amocrm');
            return ['success' => false, 'message' => 'Ошибка сохранения: ' . $e->getMessage()];
        }
    }

    /**
     * GET /admin/plugin/amocrm-widget — Widget API key management page
     * NB: read-only, no side-effects on GET (см. CSRF + prefetch hardening).
     */
    public function actionAmocrmWidget()
    {
        return $this->render('amocrm-widget');
    }

    /**
     * POST /admin/plugin/amocrm-widget/key — (Re)generate widget API key
     */
    public function actionAmocrmWidgetKey(): array
    {
        Yii::$app->response->format = Response::FORMAT_JSON;
        $this->requirePermission('manageSettings');
        ...
        $key = 'shk_' . bin2hex(random_bytes(24));
        Yii::$app->settings->set('amocrm', 'widget_api_key', $key);

        $userId = Yii::$app->user->id ?? 0;
        Yii::info('[AmoCRM] Widget API key regenerated by user #' . $userId, 'amocrm');
        $this->logAction(
            'amocrm_widget_key_rotated',
            'amocrm',
            null,
            'AmoCRM Widget API Key',
            'Widget API key rotated',
            null,
            null
        );
