...
class PdfController extends BaseAdminController
{
    ...
    {
        // CMP-470-Д: was querying non-existent column 'public_token' (schema only
        // has `token`, the crypto-random string already generated by
        // frontend\controllers\OrderController::actionCreate). Every call threw a
        // SQL "Unknown column" error. Fixed to use the real column.
        $order = Order::find()
            ->where(['token' => $token])
            ->one();
    ...
    {
        // CMP-470-Д: this referenced a view ('invoice-html') that doesn't exist
        // anywhere in the repo — every call threw ViewNotFoundException. TCPDF is
        // also not a composer dependency of this project, so class_exists('TCPDF')
        // is always false and this fallback path (not the 'invoice' view rendered
        // directly by actionInvoice/actionPrint) is the ONLY path actionPublic()
        // ever takes — meaning actionPublic() was a 100% guaranteed 500 in every
        // environment. Reused the existing, already-correct 'invoice' view (same
        // one actionInvoice/actionPrint already render for the identical
        // TCPDF-absent case) instead of inventing a new template.
