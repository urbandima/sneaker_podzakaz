...
.return-info-grid { display: flex; flex-direction: column; gap: 0; }
.return-info-row {
    display: flex;
...
.return-info-row:last-child { border-bottom: none; }
.return-info-label {
    font-size: 0.8rem;
    font-weight: 600;
...
.return-info-value { font-size: 0.95rem; text-align: right; }

.checklist-steps { display: flex; flex-direction: column; gap: 0.75rem; }
.checklist-step {
    display: flex;
    align-items: center;
    gap: 1rem;
...
.checklist-step--done .checklist-step-number { background: #22c55e; color: #fff; }
.checklist-step-icon { font-size: 1.4rem; color: var(--admin-text-secondary); flex-shrink: 0; }
.checklist-step--done .checklist-step-icon { color: #22c55e; }
.checklist-step-content { flex: 1; }
.checklist-step-label { font-weight: 600; font-size: 0.9rem; }
.checklist-step-meta { font-size: 0.75rem; color: var(--admin-text-secondary); margin-top: 0.25rem; }
.checklist-step-action { flex-shrink: 0; }
...
function markStepDone(returnId, stepKey, btn) {
    ...
    fetch('<?= Url::to(['/admin/return/update-step']) ?>', {
        method: 'POST',
        headers: {
    ...
    .then(res => {
        if (res.success) {
            ...
            // Update icon
            const iconEl = stepEl.querySelector('.checklist-step-icon i');
            if (iconEl) iconEl.className = 'bi bi-check-circle-fill';
            // Update dot number
            const numEl = stepEl.querySelector('.checklist-step-number');
            if (numEl) numEl.style.background = '#22c55e';
            // Replace button with badge
            const actionEl = stepEl.querySelector('.checklist-step-action');
            actionEl.innerHTML = '<span class="admin-badge admin-badge-success"><i class="bi bi-check2-circle"></i> Готово</span>';
            // Add meta
            const contentEl = stepEl.querySelector('.checklist-step-content');
            const meta = document.createElement('div');
            meta.className = 'checklist-step-meta';
            meta.innerHTML = '<i class="bi bi-calendar3"></i> ' + (res.date || 'только что') + (res.author ? ' &bull; <i class="bi bi-person"></i> ' + res.author : '');
