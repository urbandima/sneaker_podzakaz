...
<div class="product-view admin-product-sheet">
    ...
    <!-- Краткая сводка -->
    <div class="product-summary-grid mb-4">
        <div class="summary-card">
            <div class="summary-label">Статус</div>
            <div class="summary-value <?= $product->is_active ? 'text-success' : 'text-muted' ?>">
                <?= $product->is_active ? 'В продаже' : 'Не активен' ?>
            </div>
            <small>Показывается на сайте</small>
        </div>
        <div class="summary-card">
            <div class="summary-label">Цена</div>
            <div class="summary-value"><?= Yii::$app->formatter->asCurrency($product->price, 'BYN') ?></div>
            <small>Магазинная стоимость</small>
        </div>
        <div class="summary-card">
            <div class="summary-label">Продано</div>
            <div class="summary-value"><?= $product->orders_count ?? 0 ?></div>
            <small>Заказов с товаром</small>
        </div>
        <div class="summary-card">
            <div class="summary-label">Обновлён</div>
            <div class="summary-value"><?= Yii::$app->formatter->asDatetime($product->updated_at) ?></div>
            <small>Дата последнего изменения</small>
        </div>
    </div>
...
.admin-product-sheet .product-summary-grid {
    display: grid;
    grid-template-columns: repeat(auto-fit, minmax(220px, 1fr));
    gap: 1rem;
}
.admin-product-sheet .summary-card {
    background: #111827;
    color: #fff;
    border-radius: 16px;
    padding: 1.25rem;
...
.admin-product-sheet .summary-label {
...
.admin-product-sheet .summary-value {
    font-size: 1.35rem;
    font-weight: 700;
}
.admin-product-sheet .summary-card small {
    opacity: .6;
}
.admin-product-sheet .card {
    border-radius: 16px;
    border: 1px solid #e5e7eb;
    box-shadow: 0 8px 24px rgba(15,23,42,0.05);
}
.admin-product-sheet .card-header {
    border-bottom: 1px solid #f1f5f9;
}
.admin-product-sheet .table-sm th,
.admin-product-sheet .table-sm td {
