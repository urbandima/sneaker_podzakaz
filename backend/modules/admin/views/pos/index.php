...
?>
<style>
.pos-grid{display:grid;grid-template-columns:1fr 420px;gap:1rem;height:calc(100vh - 90px)}
.pos-left,.pos-right{background:var(--admin-surface);border-radius:var(--admin-radius-lg);padding:1rem;border:1px solid var(--admin-border);overflow:hidden;display:flex;flex-direction:column}
.pos-tabs{display:flex;gap:0.5rem;margin-bottom:1rem;border-bottom:2px solid var(--admin-border);padding-bottom:0.5rem}
.pos-tab{padding:0.5rem 1rem;border:none;background:none;cursor:pointer;font-weight:600;color:var(--admin-text-secondary);border-bottom:2px solid transparent;margin-bottom:-2px}
.pos-tab.active{color:var(--admin-accent);border-bottom-color:var(--admin-accent)}
.pos-filters{display:flex;gap:0.5rem;flex-wrap:wrap;margin-bottom:1rem;padding-bottom:0.75rem;border-bottom:1px solid var(--admin-border)}
.pos-filter-select,.pos-search{padding:0.5rem;border:1px solid var(--admin-border);border-radius:var(--admin-radius);font-size:0.875rem;background:var(--admin-surface)}
.pos-search{flex:1}
.pos-size-filters{display:flex;gap:0.25rem;flex-wrap:wrap;margin-bottom:0.75rem}
.pos-size-btn{padding:0.25rem 0.5rem;border:1px solid var(--admin-border);border-radius:4px;background:var(--admin-surface);font-size:0.75rem;cursor:pointer;transition:all 0.15s}
.pos-size-btn:hover{border-color:var(--admin-accent)}
.pos-size-btn.active{border-color:var(--admin-accent);background:var(--admin-accent);color:white}
.pos-products{display:grid;grid-template-columns:repeat(auto-fill,minmax(130px,1fr));gap:0.75rem;overflow-y:auto;flex:1}
.pos-product{background:var(--admin-bg);border-radius:var(--admin-radius);padding:0.75rem;cursor:pointer;text-align:center;border:2px solid transparent;transition:all 0.15s}
.pos-product:hover{border-color:var(--admin-accent);transform:translateY(-2px)}
.pos-product.out{opacity:0.5;cursor:not-allowed}
.pos-product img{width:100%;height:80px;object-fit:cover;border-radius:var(--admin-radius);margin-bottom:0.5rem;background:var(--admin-border-light)}
.pos-product .name{font-weight:600;font-size:0.75rem;line-height:1.2;margin-bottom:0.25rem;overflow:hidden;display:-webkit-box;-webkit-line-clamp:2;-webkit-box-orient:vertical}
.pos-product .price{font-weight:700;color:var(--admin-accent);font-size:0.85rem}
.pos-product .stock{font-size:0.7rem;color:var(--admin-text-secondary);margin-top:0.25rem}
.pos-customer{border-bottom:1px solid var(--admin-border);padding-bottom:0.75rem;margin-bottom:0.75rem;display:flex;justify-content:space-between;align-items:center}
.pos-customer-info{flex:1}
.pos-customer-name{font-weight:600;margin-bottom:0.25rem}
.pos-customer-phone{font-size:0.75rem;color:var(--admin-text-secondary)}
.pos-customer-btn{padding:0.25rem 0.5rem;font-size:0.75rem;background:none;border:1px solid var(--admin-border);border-radius:var(--admin-radius);cursor:pointer;color:var(--admin-accent)}
.pos-cart{flex:1;overflow-y:auto;display:flex;flex-direction:column;gap:0.5rem}
.pos-cart-empty{text-align:center;padding:3rem 1rem;color:var(--admin-text-secondary)}
.pos-cart-item{display:flex;gap:0.75rem;padding:0.75rem;background:var(--admin-bg);border-radius:var(--admin-radius);align-items:center}
.pos-cart-item img{width:45px;height:45px;object-fit:cover;border-radius:var(--admin-radius);background:var(--admin-border-light)}
.pos-cart-item-info{flex:1;min-width:0}
.pos-cart-item-name{font-weight:600;font-size:0.8rem;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.pos-cart-item-meta{font-size:0.7rem;color:var(--admin-text-secondary)}
.pos-cart-item-qty{display:flex;align-items:center;gap:0.5rem}
.pos-qty-btn{width:22px;height:22px;border:1px solid var(--admin-border);border-radius:4px;background:var(--admin-surface);cursor:pointer;display:flex;align-items:center;justify-content:center;font-size:0.8rem}
.pos-cart-item-total{font-weight:700;min-width:50px;text-align:right;font-size:0.85rem}
.pos-totals{border-top:1px solid var(--admin-border);padding-top:0.75rem;margin-top:0.75rem}
.pos-total-row{display:flex;justify-content:space-between;padding:0.25rem 0;font-size:0.875rem}
.pos-total-row.grand{font-size:1.2rem;font-weight:700;border-top:1px solid var(--admin-border);margin-top:0.5rem;padding-top:0.5rem}
.pos-discount{margin:0.75rem 0;padding:0.75rem;background:var(--admin-bg);border-radius:var(--admin-radius)}
.pos-discount-input{display:flex;gap:0.5rem;margin-top:0.5rem}
.pos-discount-input input{flex:1;padding:0.5rem;border:1px solid var(--admin-border);border-radius:var(--admin-radius);font-size:0.875rem}
.pos-discount-input select{padding:0.5rem;border:1px solid var(--admin-border);border-radius:var(--admin-radius);font-size:0.875rem}
.pos-payment{margin:0.75rem 0}
.pos-payment-grid{display:grid;grid-template-columns:1fr 1fr;gap:0.5rem;margin-bottom:0.5rem}
.pos-payment-btn{padding:0.75rem;border:2px solid var(--admin-border);border-radius:var(--admin-radius);background:var(--admin-surface);cursor:pointer;text-align:center;font-size:0.875rem;transition:all 0.15s}
.pos-payment-btn.active{border-color:var(--admin-accent);background:var(--admin-accent);color:white}
.pos-payment-split{display:none;gap:0.5rem;margin-top:0.5rem}
.pos-payment-split.active{display:flex}
.pos-payment-split input{flex:1;padding:0.5rem;border:1px solid var(--admin-border);border-radius:var(--admin-radius);font-size:0.875rem}
.pos-checkout-btn{width:100%;padding:1rem;background:var(--admin-accent);color:white;border:none;border-radius:var(--admin-radius);font-size:1rem;font-weight:600;cursor:pointer;transition:all 0.15s}
.pos-checkout-btn:hover{background:var(--admin-accent-dark)}
.pos-checkout-btn:disabled{opacity:0.5;cursor:not-allowed}
.pos-modal{position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:9999;display:none;align-items:center;justify-content:center}
.pos-modal.active{display:flex}
.pos-modal-box{background:var(--admin-surface);border-radius:var(--admin-radius-lg);width:90%;max-width:600px;max-height:80vh;overflow:hidden;display:flex;flex-direction:column}
.pos-modal-header{padding:1rem;border-bottom:1px solid var(--admin-border);display:flex;justify-content:space-between;align-items:center}
.pos-modal-header h3{margin:0}
.pos-modal-close{background:none;border:none;font-size:1.5rem;cursor:pointer;color:var(--admin-text-secondary)}
.pos-modal-body{flex:1;overflow-y:auto;padding:1rem}
.pos-customer-row{padding:0.75rem;background:var(--admin-bg);border-radius:var(--admin-radius);margin-bottom:0.5rem;cursor:pointer;display:flex;justify-content:space-between;align-items:center;transition:all 0.15s}
.pos-customer-row:hover{background:var(--admin-border-light)}
.pos-order-row{padding:1rem;background:var(--admin-bg);border-radius:var(--admin-radius);margin-bottom:0.75rem;cursor:pointer;border:2px solid transparent;transition:all 0.15s}
.pos-order-row:hover{border-color:var(--admin-accent)}
.pos-order-header{display:flex;justify-content:space-between;align-items:center;margin-bottom:0.5rem}
.pos-order-number{font-weight:700;font-size:1rem}
.pos-order-status{padding:0.25rem 0.5rem;border-radius:4px;font-size:0.75rem;font-weight:600}
.pos-order-status.paid{background:#dbeafe;color:#1e40af}
.pos-order-status.ready{background:#d1fae5;color:#065f46}
.pos-order-items{font-size:0.875rem;color:var(--admin-text-secondary);margin-bottom:0.5rem}
.pos-order-total{font-weight:700;font-size:1.1rem;color:var(--admin-accent)}
.pos-actions{display:flex;gap:0.5rem;margin-top:0.75rem}
@media(max-width:1200px){.pos-grid{grid-template-columns:1fr;height:auto}.pos-left{height:50vh}.pos-right{max-height:50vh}}
</style>
