 ...
 *
 * НАЗНАЧЕНИЕ:
 * Построение всех типов фильтров каталога: бренды, категории,
 * характеристики, размеры, цвета, цены. Умное сужение фильтров.
 *
 * ФУНКЦИИ:
 * - buildFilters(): построение всех фильтров с кэшированием
 * - buildBrandsFilter(): фильтр по брендам
 * - buildCategoriesFilter(): фильтр по категориям
 * - buildPriceRange(): фильтр по цене
 * - buildSizesFilter(): фильтр по размерам
 * - buildColorsFilter(): фильтр по цветам
 * - buildCharacteristicsFilter(): фильтр по характеристикам
 *
 * ОСОБЕННОСТИ:
...
namespace app\backend\modules\catalog\services\Catalog;
...
use app\backend\modules\catalog\models\ProductColor;
...
class FilterBuilder
{
        ...
        foreach ($currentFilters as $key => $value) {
            if (strpos($key, 'char_') === 0 && !in_array($key, $exclude)) {
                ...
                if (is_array($value) && count($value) > 0) {
                    ...
                    $characteristic = \app\backend\modules\catalog\models\Characteristic::findOne($charId);
    ...
    public static function formatActiveFilters(array $filters, ?callable $urlGenerator = null): array
    {
        ...
        if (!empty($filters['price_from']) || !empty($filters['price_to'])) {
            ...
            if (!empty($filters['price_from'])) {
                $label .= 'от ' . $filters['price_from'] . ' ';
            }
            if (!empty($filters['price_to'])) {
                $label .= 'до ' . $filters['price_to'];
            }
    ...
    protected static function buildQueryStringUrl(array $filters): string
    {
        $params = [];
        if (!empty($filters['brands'])) {
            $params['brands'] = implode(',', $filters['brands']);
        }
        if (!empty($filters['categories'])) {
            $params['categories'] = implode(',', $filters['categories']);
        }
        if (!empty($filters['sizes'])) {
            $params['sizes'] = implode(',', $filters['sizes']);
        }
        if (!empty($filters['size_system'])) {
            $params['size_system'] = $filters['size_system'];
        }
        if (!empty($filters['price_from'])) {
        ...
        if (!empty($filters['colors'])) {
            $params['colors'] = implode(',', $filters['colors']);
        }
        if (!empty($filters['discount_any'])) {
            $params['discount_any'] = 1;
        }
        if (!empty($filters['discount_range'])) {
            $params['discount_range'] = is_array($filters['discount_range']) ? implode(',', $filters['discount_range']) : $filters['discount_range'];
        }
        if (!empty($filters['rating'])) {
            $params['rating'] = $filters['rating'];
        }
        if (!empty($filters['conditions'])) {
            $params['conditions'] = is_array($filters['conditions']) ? implode(',', $filters['conditions']) : $filters['conditions'];
        }
