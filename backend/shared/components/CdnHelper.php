...
/**
 * CdnHelper — Компонент работы с CDN
 * 
 * НАЗНАЧЕНИЕ:
 * Интеграция с CDN для доставки статических ресурсов:
 * изображения, CSS, JS, шрифты. Ускорение загрузки.
 * 
 * ПОДДЕРЖИВАЕМЫЕ CDN:
 * - CloudFlare
 * - BunnyCDN
 ...
 * 
 * НАСТРОЙКИ:
 * - enabled: включён ли CDN
 * - baseUrl: базовый URL CDN
 * - type: тип CDN (cloudflare, bunny, cloudfront, keycdn, custom)
 * - extensions: расширения файлов для CDN
 * 
 * ИСПОЛЬЗОВАНИЕ:
 * ```php
 * $url = Yii::$app->cdn->getUrl('/images/product.jpg');
 * $url = Yii::$app->cdn->getUrlWithSize('/images/product.jpg', 300, 300);
 * ```
 * 
 * ОСОБЕННОСТИ:
 ...
 * 'components' => [
 *     'cdn' => [
 *         'class' => 'app\backend\shared\components\CdnHelper',
 *         'enabled' => true,
