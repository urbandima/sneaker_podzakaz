...
/**
 * CacheManager — Компонент централизованного кэширования
 *
 * НАЗНАЧЕНИЕ:
 * Управление кэшированием с поддержкой тегов для массовой инвалидации.
 ...
 * });
 * CacheManager::invalidateTags(['filters', 'products']);
 * ```
 *
 * ОСОБЕННОСТИ:
 * - Автоопределение Redis или File кэша
 * - TTL по умолчанию для разных типов данных
 * - Batch инвалидация по тегам
 */
...
class CacheManager extends Component
{
    ...
    public const TAG_CATALOG = 'catalog';
     ...
     */
    public const TTL_SHORT = 300;      // 5 минут (счётчики, поиск)
    public const TTL_MEDIUM = 1800;    // 30 минут (фильтры, каталог)
    public const TTL_LONG = 3600;      // 1 час (справочники, статика)
    public const TTL_VERY_LONG = 86400; // 24 часа (редко меняющиеся данные)
    ...
    public static function warmupFilters()
    {
        ...
        $callback = function () {
            $controller = new \app\modules\catalog\controllers\CatalogController('catalog', Yii::$app);
