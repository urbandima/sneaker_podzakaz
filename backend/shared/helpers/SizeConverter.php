...
/**
 * SizeConverter — Конвертер размеров обуви
 * 
 * НАЗНАЧЕНИЕ:
 * Конвертация размеров обуви между системами: EU, US, UK, CM.
 * Учёт особенностей брендов (маломерки/большемерки).
 * 
 * МЕТОДЫ:
 * - euToUs(): конвертация EU в US
 * - euToUk(): конвертация EU в UK
 * - euToCm(): конвертация EU в CM
 * - convert(): универсальная конвертация
 * - getBrandAdjustment(): корректировка по бренду
 * 
 * ТАБЛИЦА РАЗМЕРОВ:
 * - EU => [US_M, US_W, UK, CM]
 * - Диапазон: 35-47 EU
 * 
 * КОРРЕКТИРОВКИ БРЕНДОВ:
 * - Nike: стандартный размер
 * - Adidas: маломерят на 0.5
 * - New Balance: большемерки
 * 
 * ИСПОЛЬЗОВАНИЕ:
 * ```php
 * $usSize = SizeConverter::euToUs(42);
 * $adjusted = SizeConverter::getBrandAdjustment('Adidas', 42);
 * ```
 */
