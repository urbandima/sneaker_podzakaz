...
<div class="container" style="max-width:560px;margin:60px auto;padding:0 16px 60px">
    ...
    <p style="color:var(--color-text-secondary);margin-bottom:28px">
    ...
    <form id="findOrdersForm" style="display:flex;flex-direction:column;gap:16px">
        ...
        <div style="text-align:center;color:var(--color-text-secondary);font-size:.875rem">или</div>
        <div>
...
document.getElementById('findOrdersForm').addEventListener('submit', function(e) {
    ...
    .then(data => {
        ...
        const rows = data.orders.map(o => `
            <div style="border:1px solid var(--color-border);border-radius:8px;padding:16px 20px;margin-bottom:12px;display:flex;align-items:center;justify-content:space-between;gap:12px">
                <div>
                    <div style="font-weight:600">Заказ #${o.id}</div>
                    <div style="font-size:.875rem;color:var(--color-text-secondary)">${o.created_at} · ${o.total}</div>
        ...
        document.getElementById('trackingResults').innerHTML = `<h2 style="font-size:1rem;font-weight:600;margin-bottom:16px">Найдено заказов: ${data.count}</h2>${rows}`;
