...
document.addEventListener('DOMContentLoaded', function () {
    ...
    var csrfToken = document.querySelector('meta[name="csrf-token"]') ? getCsrfToken() : '';
...
function updateProductMeta(id, field, value) {
    fetch('/admin/seo/update-product-meta', {
        ...
        headers: {
            'Content-Type': 'application/x-www-form-urlencoded',
            'X-CSRF-Token': (typeof yii !== 'undefined' ? yii.getCsrfToken() : (getCsrfToken()))
        },
...
document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('.js-flag').forEach(function (flag) {
        flag.addEventListener('change', function () {
            ...
            var csrfToken = config ? config.dataset.csrfToken : (getCsrfToken());
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    fetch('/admin/exchange-rate/current')
        ...
        .then(data => {
            if (data.rate) {
                const rateEl = document.getElementById('cny-rate');
                const updatedEl = document.getElementById('cny-updated');
