...
class m260404_102900_create_product_tag_tables extends \yii\db\Migration
{
    ...
    public function safeUp()
    {
        ...
        // product_tag / product_tag_assignment уже существуют на части окружений
        // (созданы вручную вне миграций) — создаём только если ещё нет, чтобы
        // миграция была применима и на свежей БД, и на текущей.
        if ($this->db->schema->getTableSchema('{{%product_tag}}', true) === null) {
        ...
        if ($this->db->schema->getTableSchema('{{%product_tag_assignment}}', true) === null) {
